...
elseif ($method === "GET" && $action === "orders") {
    verifyAdmin($pdo);
    $stmt = $pdo->prepare("SELECT o.*, u.name as patient_name, u.email as patient_email FROM orders o LEFT JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC"); $stmt->execute();
